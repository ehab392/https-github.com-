(() => {
  ...
  // ---------- إشعار الطلبات الجديدة (داخل لوحة التحكم) ----------
  let lastSeenOrderId = Number(localStorage.getItem('hb_admin_last_order') || 0);
  ...
  function notifyBrowser(count) {
    ...
    try {
      new Notification('طلب جديد في حجة بتول', {
        body: count === 1 ? 'وصل طلب جديد إلى المتجر' : `وصلت ${count} طلبات جديدة إلى المتجر`,
  ...
  function setOrdersTabBadge(count) {
    const btn = document.querySelector('#tabs button[data-tab="orders"]');
    if (!btn) return;
    btn.textContent = count > 0 ? `الطلبات (${count})` : 'الطلبات';
  }
  ...
  async function pollNewOrders() {
    try {
      ...
      if (lastSeenOrderId && maxId > lastSeenOrderId) {
        const newCount = orders.filter((o) => o.id > lastSeenOrderId).length;
        toast(`🔔 وصل ${newCount === 1 ? 'طلب جديد' : newCount + ' طلبات جديدة'}!`);
        notifyBrowser(newCount);
        setOrdersTabBadge(newCount);
      }
      if (!lastSeenOrderId && maxId) {
        lastSeenOrderId = maxId; // أول مرة: لا نُنبّه على طلبات قديمة، فقط نسجّل آخر رقم
        localStorage.setItem('hb_admin_last_order', String(maxId));
      }
    } catch (_) { /* تجاهل أخطاء الاستطلاع الصامت */ }
  }
  ...
  async function loadOrders() {
    ...
    try {
      ...
      const maxId = S.orders.reduce((m, o) => Math.max(m, o.id), 0);
      if (maxId > lastSeenOrderId || !lastSeenOrderId) {
      ...
      const active = S.orders.filter((o) => o.status === 'new' || o.status === 'confirmed' || o.status === 'shipped');
      const finished = S.orders.filter((o) => o.status === 'done' || o.status === 'cancelled');

      box.innerHTML = `
        <div class="order-section">
          <h3 class="order-section-title">🆕 الطلبات الجديدة والنشطة <span class="count">${active.length}</span></h3>
          ${active.length ? active.map(orderCard).join('') : '<p class="empty">لا توجد طلبات نشطة حاليًا.</p>'}
        </div>
        <div class="order-section">
          <h3 class="order-section-title done">✅ الطلبات المكتملة والملغاة <span class="count">${finished.length}</span></h3>
          ${finished.length ? finished.map(orderCard).join('') : '<p class="empty">لا توجد طلبات مكتملة بعد.</p>'}
        </div>`;
  ...
  (async () => {
    ...
    setInterval(pollNewOrders, 25000);
  })();
})();
